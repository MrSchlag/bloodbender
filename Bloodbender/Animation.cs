...
namespace Bloodbender
{
    public class Animation
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 position, float rotation, SpriteEffects spriteEffect, Vector2 scale, float height = 0.0f)
        {
            ...
            if (!isDepthForce)
                depth = ((position.Y + texture.Height * scale.Y) - (Bloodbender.ptr.camera.position.Y - (Bloodbender.ptr.camera.height / 2))) / 10000.0f; // ATTENTION A CORRIGER SI MANIPULATION SUR LE SCALE, A CORRIGER QUAND PhysicObj FINI, depth responsable du non  affichage de sprite sur les negatif
